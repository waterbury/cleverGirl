...
void loop() {
      ...
      while(1){
       drive    = Y_translate( analogRead(Y_CTRL) ); //Returns -100 to 100 for speed percentage. Reverse to Forward, respectfully.
       if (drive < 5 && drive > -5)
        drive = 0;
       carRotation = Z_translate( analogRead(Z_CTRL) ); //Returns -100 to 100 for Car Rotation. Left, to Right, respectfully.
       if (carRotation < 45 && carRotation > -45)
        carRotation = 0;
       
       carRotation *= turnWeight;// Adjusts roation by pre-definded weight.
       strafe      = X_translate( analogRead(X_CTRL) );
       

      
       

      strafe = 0; //disable strafe
     // drive *= .5;
/*      
      fl     = drive + strafe  + carRotation  ;
      bl     = drive - strafe  + carRotation  ;
      fr     = drive - strafe  - carRotation  ;
      br     = drive + strafe  - carRotation  ;
*/


      tankControl(drive, carRotation,carWheels);
...
//int carWheels[4] = {0,0,0,0};
  ...
  //car is not moving 
  if (carSpeed == 0){
    carWheels[0] = 0; //left wheels go full speed
    carWheels[1] = 0;
    carWheels[2] = 0; //left wheels go full speed
    carWheels[3] = 0;
   }
  ...
  //car is not rotating
   ...
   //all wheels move forward at speed of vehicle  
    carWheels[0] = carSpeed; //left wheels go full speed
    carWheels[1] = carSpeed;
    carWheels[2] = carSpeed; //left wheels go full speed
    carWheels[3] = carSpeed;
   }
  ...
  //Moving Forward, or backwards
   ...
   if(carRotation > 0){
    carWheels[1] = carSpeed; //left wheels go full speed
    carWheels[3] = carSpeed;
    carWheels[0] = carSpeed * ( carRotation / 100.0) * -1 ; //rightWheel goes carRotation of a percent slower than left 
    carWheels[2] = carSpeed * ( carRotation / 100.0) * -1 ; //rightWheel goes carRotation of a percent slower than left 
    }

   //vehicle is turning left
   else if (carRotation < 0){
    carWheels[0] = carSpeed; //right wheels go full speed
    carWheels[2] = carSpeed; //right wheels go full speed
    carWheels[1] = carSpeed * ( (carRotation * -1 ) / 100.0) *-1 ; //leftWheels goes carRotation of a percent slower than right 
    carWheels[3] = carSpeed * ( (carRotation * -1 ) / 100.0) *-1 ; //leftWheels goes carRotation of a percent slower than right 
    }
  }
